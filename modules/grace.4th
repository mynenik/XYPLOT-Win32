...
create grace_xydata[[ MAXGRACEPTS 2 * DFLOATS allot
create grace_pattrs[[ MAXPLOTS NATTRIBUTES * cells allot   \ plot attributes
: ]]xy ( a row col -- a2 ) >r 2 * r> + dfloats + ;
...
: parse_world_coords ( a u -- | parse world coordinates )
    ...
    2dup s" xmin" search IF
      5 /string bl skip $>f ( >float IF) xmin f! ( THEN) 
      2drop EXIT
    ELSE 2drop THEN

    2dup s" xmax" search IF
      5 /string bl skip $>f ( >float IF) xmax f! ( THEN) 
      2drop EXIT
    ELSE 2drop THEN

    2dup s" ymin" search IF
      5 /string bl skip $>f ( >float IF) ymin f! ( THEN) 
      2drop EXIT
    ELSE 2drop THEN

    2dup s" ymax" search IF
      5 /string bl skip $>f ( >float IF) ymax f! ( THEN) 
      2drop EXIT
    ELSE 2drop THEN
...
: parse_visibility ( a1 u1 a2 u2 -- a1 u1 )
    ...
    s" true" search
    grace_pattrs[[ grace_set @ ATTR_HIDDEN ]]pattrs !
    2drop
;
...
: parse_line_attrs ( a1 u1 a2 u2 -- a1 u1 )
    ...
    2dup s" line type" search IF
      10 /string $>s
      grace_pattrs[[ grace_set @ ATTR_LINETYPE ]]pattrs !
      2drop EXIT
    ELSE 2drop THEN

    2dup s" line linestyle" search IF
      15 /string $>s
      grace_pattrs[[ grace_set @ ATTR_LINESTYLE ]]pattrs !
      2drop EXIT
    ELSE 2drop THEN

    2dup s" line linewidth" search IF
      15 /string bl skip $>f ( >float if)
        10e f* f>d drop
        grace_pattrs[[ grace_set @ ATTR_LINEWIDTH ]]pattrs !
      ( then)
      2drop EXIT
    ELSE 2drop THEN
    ...
    2dup s" line color" search IF
      11 /string $>s
      grace_pattrs[[ grace_set @ ATTR_LINECOLOR ]]pattrs !
      2drop EXIT 
    ELSE 2drop THEN
...
: parse_symbol_attrs ( a1 u1 a2 u2 -- a1 u1 )
    ...
    2dup s" symbol color" search IF
      13 /string $>s
      grace_pattrs[[ grace_set @ ATTR_SYMBOLCOLOR ]]pattrs !
      2drop EXIT
    ELSE 2drop THEN

    2dup s" symbol size" search IF
      12 /string bl skip $>f ( >float IF)
        10e f* f>d drop
        grace_pattrs[[ grace_set @ ATTR_SYMBOLSIZE ]]pattrs !
      ( THEN)
      2drop EXIT
    ELSE 2drop THEN
    ...
    dup 1 = IF
      drop c@ [char] 0 -
      grace_pattrs[[ grace_set @ ATTR_SYMBOL ]]pattrs !
      2drop EXIT
    ELSE 2drop THEN
...
\ set is hidden
: map_grace_attr ( -- nsymbol coloridx ntype true | false )
    ...
    grace_pattrs[[ grace_set @ ATTR_SYMBOL ]]pattrs @ IF
      grace_pattrs[[ grace_set @ ATTR_LINETYPE ]]pattrs @ IF
        sym_LINE_PLUS_POINT xyp_symbol !
      ELSE
        grace_pattrs[[ grace_set @ ATTR_SYMBOLSIZE ]]pattrs @ 11 < IF
    ...
    ELSE
      ATTR_LINECOLOR
    THEN
    grace_pattrs[[ grace_set @ rot ]]pattrs @
    ...
    BEGIN
      <grfile IF
        ...
        ELSE
          ...
          grace_xydata[[ grace_set_npts @ 1 ]]xy F!
          grace_xydata[[ grace_set_npts @ 0 ]]xy F!
          1 grace_set_npts +!
        THEN
      ELSE
        2drop
        -1 EXIT   \ reached EOF before end of dataset
      THEN
      grace_set_npts @ MAXGRACEPTS >= IF
        -2 EXIT 
      THEN
    AGAIN
;
